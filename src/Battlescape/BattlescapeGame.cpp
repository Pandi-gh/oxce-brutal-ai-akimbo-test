			...
			cost.Energy + energy > bu->getEnergy()) ) )
	{
		if (!justChecking)
		{
			...
			else
			{
				...
				{
				case BA_AKIMBOSHOT: _parentState->warning("STR_TIME_UNITS_RESERVED_FOR_AKIMBO_SHOT"); break;
	...
	if (_currentAction.targeting && _save->getSelectedUnit())
	{
		...
		else if (_currentAction.sprayTargeting) // Special "spray" auto shot that allows placing shots between waypoints
		{
				...
				(!Options::battleConfirmFireMode && (int)_currentAction.waypoints.size() == maxWaypoints - 1))
			{
				...
				/** let make proper point count and avoid endless loop error,which occur by shot count > shotNumber **/
				if (_currentAction.type == BA_AKIMBOSHOT)
				{	
					numberOfShots = _currentAction.actor->getLeftHandWeapon()->getRules()->getConfigAkimbo()->shots +
						            _currentAction.actor->getRightHandWeapon()->getRules()->getConfigAkimbo()->shots;
				}
...
void BattlescapeGame::psiAttackMessage(BattleActionAttack attack, BattleUnit *victim)
{
	if (victim)
	{
		...
		{
			// show a little infobox if it's successful.  Adding selectable options for moral attack notification behaviour
